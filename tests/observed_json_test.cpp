...
TEST_CASE("to_json serializes observed state with the fixed contract shape") {
  ...
  SECTION("a default state keeps the shape with explicit nulls, no "
          "health/fabric keys") {
    const nlohmann::json expected = {
...
TEST_CASE("to_json serializes per-GPU health: omit when unset, explicit null "
          "when engaged") {
  GpuInfo gpu{
      .index = 0,
      .name = "NVIDIA H100 80GB HBM3",
